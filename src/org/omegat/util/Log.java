...
public class Log {
	...
	static {
		...
		if (usersLogSettings.exists()) {
			...
			try (InputStream in = new FileInputStream(usersLogSettings)) {
				...
				LOGGER.log(Level.SEVERE, "Failed to load logger settings from user home dir", e);
			}
		}
		if (!loaded) {
		...
		if (handlers != null) {
			...
			for (String hn : hs) {
				...
				try {
					...
					LOGGER.log(Level.SEVERE, "Error in logger init" + ex);
				}
			}
		}
	}
	 ...
	 * @param parameters Parameters for the message. These are inserted by using StaticUtils.format.
	 * @author Henry Pijffers (henry.pijffers@saxnot.com)
	 */
	 ...
	 * @param parameters Parameters for the error message. These are inserted by using
	 *                   StaticUtils.format.
	 * @author Henry Pijffers (henry.pijffers@saxnot.com)
